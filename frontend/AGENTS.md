...
**Geofence Draw Mode:** enabled via SETTINGS → GEOFENCE DRAW. When active, the map stays in a locked interaction state and the top-centre `GeofenceDrawToolbar` collects the fence name plus vertex count. Left-click adds vertices to the draft polygon. Draft geofences render green; active saved fences render amber; inactive saved fences render muted gray. Saved geofences remain visible regardless of draw-mode state. Exiting draw mode does not clear a non-empty draft; only FINISH (successful `POST /api/geofences`) or CANCEL clears it. The toolbar only enables FINISH when the draft has at least 3 vertices and a non-empty name. Geofence mode is mutually exclusive with Mission Planning and Driver Mode.
